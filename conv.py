# ---------------------------------------------------------------------------------------------#
#                                                                                              #
#                  Convolutional Neural Network - PyTorch - Double Digit                       #
#                                                                                              #                                                                                         #                                                                                             #
# ---------------------------------------------------------------------------------------------#
...
class CNN(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.conv2d_1(x)  # 40x26
        x = self.relu(x)      
        x = self.maxpool2d(x) # 20x13
        x = self.conv2d_2(x)  # 18x11
        x = self.relu(x)
        x = self.maxpool2d(x) # 9x5
        x = self.flatten(x)   # 64x9x5 = 2880
        x = self.linear1(x)   
        x = self.dropout(x)
        x = self.linear2(x)
